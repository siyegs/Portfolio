...
A published-quality open-source library for lossless video trimming on React Native and Expo. Most trim libraries bundle ffmpeg (tens of MB of binary, a quality-degrading re-encode on every clip, and a dependency on ffmpeg-kit, which was retired in 2025). This one uses only the platform's native APIs - AVAssetExportSession passthrough on iOS, MediaExtractor and MediaMuxer on Android - to stream-copy the frames, so the trim is lossless, near-instant, and adds essentially no binary weight. It also fixes a subtle Android audio/video desync that most passthrough trimmers ship with, by rebasing both tracks to a single shared keyframe origin, and it is honest about the iOS-versus-Android precision tradeoff. Written as an Expo module with a typed TypeScript API, New Architecture support, an example app, unit-tested timing logic, and CI. Source at https://github.com/siyegs/react-native-lossless-trim.
